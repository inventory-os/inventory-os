...
export async function GET(request: Request, { params }: { params: Promise<{ id: string; fileId: string }> }) {
  ...
  const fileRecord = await getAssetFileById(id, fileId)
  ...
  try {
    const content = await readStoredAssetFile(fileRecord.storageKey)
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string; fileId: string }> }) {
  const { id, fileId } = await params
  const deleted = await deleteAssetFileRecord(id, fileId)

  if (!deleted) {
    return NextResponse.json({ error: "File not found" }, { status: 404 })
  }

  await removeStoredAssetFile(deleted.storageKey)
