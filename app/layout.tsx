...
export default async function RootLayout({
  ...
  const trustErrorTitle = !domainTrusted ? 'Trusted domain configuration is invalid' : 'Trusted proxy configuration is invalid'
        ...
        {trustOk ? (
          ...
          <main className="flex min-h-screen items-center justify-center bg-background p-6">
            <div className="w-full max-w-2xl rounded-2xl border border-destructive/30 bg-card p-6 shadow-sm">
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-destructive">Proxy Configuration Error</p>
              ...
              <div className="mt-4 rounded-lg border border-border bg-muted/30 p-3">
                ...
                <p className="mt-2 text-[11px] text-muted-foreground">
                  Effective sources — Domains: {security.trustedDomainsSource.toUpperCase()} · Proxies: {security.trustedProxiesSource.toUpperCase()}
                </p>
              </div>
            </div>
          </main>
