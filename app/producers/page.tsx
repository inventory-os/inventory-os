...
export default function ProducersPage() {
  ...
  const importProducer = async () => {
    ...
    if (!response.ok) {
      ...
      setMessage(await getErrorMessage(response, t("producersImportFailed")))
      return
    }
  ...
  const createProducerManually = async () => {
    ...
    if (!response.ok) {
      setMessage(await getErrorMessage(response, t("producersManualCreateFailed")))
      return
    }
  ...
  const saveEditedProducer = async () => {
    ...
    setSaving(true)
    setMessage(null)
    const response = await fetch(`/api/producers/${editingProducer.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: editForm.name,
    ...
    if (!response.ok) {
      setMessage(await getErrorMessage(response, t("producersUpdateFailed")))
      return
    }
  ...
  const removeProducer = async (producer: Producer) => {
    ...
    setMessage(null)
    const response = await fetch(`/api/producers/${producer.id}`, { method: "DELETE" })
    setDeletingId(null)

    if (!response.ok) {
      setMessage(await getErrorMessage(response, t("producersDeleteFailed")))
      return
    }
              ...
              <CardContent className="space-y-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold" title={producer.name}>{producer.name}</p>
                    <p className="truncate text-xs text-muted-foreground" title={producer.domain}>{producer.domain}</p>
                  </div>
                </div>
                ...
                <div className="space-y-1 text-xs">
                  <a href={producer.websiteUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 truncate hover:text-primary" title={producer.websiteUrl}>
                    <Globe className="size-3.5 shrink-0" />
                    {producer.websiteUrl}
                  </a>
                  <a href={producer.sourceUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 truncate text-muted-foreground hover:text-primary" title={producer.sourceUrl}>
      ...
      <Dialog open={openEdit} onOpenChange={setOpenEdit}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={saveEditedProducer} disabled={saving || editForm.name.trim().length < 2 || !editForm.websiteUrl.startsWith("http")}>
