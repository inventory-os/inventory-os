...
function renderRichMessage(input: string): string {
  ...
  html = html.replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g, '<a href="$2" target="_blank" rel="noreferrer" class="underline underline-offset-2">$1</a>')
...
export default function QrLandingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [payload, setPayload] = useState<QrResolvePayload | null>(null)
  ...
  useEffect(() => {
    ...
    async function resolve() {
      setLoading(true)
      const response = await fetch(`/api/qr/${id}`, { cache: "no-store" })
      ...
      const data = (await response.json()) as QrResolvePayload
      if (cancelled) {
        return
      }
        ...
        <CardHeader>
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <CardDescription>
                {payload.entity.type === "asset" ? t("qrPublicAssetLabel") : t("qrPublicLocationLabel")} · {payload.entity.id}
              </CardDescription>
            </div>
          ...
          {payload.public?.logoUrl ? (
            <div className="rounded-2xl border border-border/60 bg-card p-5 shadow-sm">
              <div className="flex h-24 items-center justify-center">
                <img src={payload.public.logoUrl} alt="Owner logo" className="h-16 w-auto max-w-[240px] object-contain" />
              </div>
            </div>
          ) : (
            ...
            <div className="rounded-xl border border-border/60 bg-muted/20 px-4 py-3">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">{t("qrPublicAddress")}</p>
                  <p className="text-sm font-semibold leading-snug">{payload.public.selectedAddress.label}</p>
                  <p className="mt-0.5 text-sm text-muted-foreground leading-relaxed">{payload.public.selectedAddress.fullAddress}</p>
                </div>
              </div>
            </div>
          ) : null}
          ...
          <div className="flex flex-wrap gap-2.5">
            {payload.public?.websiteUrl ? (
              <Button asChild size="icon" variant="outline" className="rounded-full" title={t("qrPublicWebsite")}>
                <a href={ensureExternalUrl(payload.public.websiteUrl)} target="_blank" rel="noreferrer" aria-label={t("qrPublicWebsite")}>
            ...
            {(payload.public?.extraLinks ?? []).map((entry) => (
              <Button key={`${entry.label}-${entry.url}`} asChild size="icon" variant="outline" className="rounded-full" title={entry.label || t("qrPublicOpenLink")}>
                <a href={ensureExternalUrl(entry.url)} target="_blank" rel="noreferrer" aria-label={entry.label || t("qrPublicOpenLink")}>
