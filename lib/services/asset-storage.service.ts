...
function getQrLogoStorageDir(): string {
  return path.join(getRootStorageDir(), "qr-logos")
}
...
export async function storeQrLogoFile(params: {
  originalName: string
  content: Buffer
  extension: string
}): Promise<{ fileName: string; absolutePath: string; sizeBytes: number }> {
  const root = getQrLogoStorageDir()
  await mkdir(root, { recursive: true })

  const safeBaseName = sanitizeFileName(path.basename(params.originalName, path.extname(params.originalName)))
  const normalizedExt = params.extension.startsWith(".")
    ? params.extension.toLowerCase()
    : `.${params.extension.toLowerCase()}`
  const fileName = `qr-logo-${Date.now()}-${randomUUID().slice(0, 8)}-${safeBaseName}${normalizedExt}`
...
export async function readStoredQrLogoFile(fileName: string): Promise<Buffer> {
  const root = getQrLogoStorageDir()
